...
/** admin/jqadm/type/popup/fields
 * List of popup type columns that should be displayed in the list view
 *
 * Changes the list of popup type columns shown by default in the popup type
 * list view. The columns can be changed by the editor as required within the
 * administraiton interface.
 *
 * The names of the colums are in fact the search keys defined by the managers,
 * e.g. "popup.type.id" for the popup type ID.
 *
 * @param array List of field names, i.e. search keys
 * @since 2017.10
 * @category Developer
 */
$default = ['popup.type.domain', 'popup.type.status', 'popup.type.code', 'popup.type.label'];
$default = $this->config( 'admin/jqadm/type/popup/fields', $default );
$fields = $this->session( 'aimeos/admin/jqadm/type/popup/fields', $default );
...
$columnList = [
	'popup.type.id' => $this->translate( 'admin', 'ID' ),
	'popup.type.domain' => $this->translate( 'admin', 'Domain' ),
	'popup.type.status' => $this->translate( 'admin', 'Status' ),
	'popup.type.code' => $this->translate( 'admin', 'Code' ),
	'popup.type.label' => $this->translate( 'admin', 'Label' ),
	'popup.type.position' => $this->translate( 'admin', 'Position' ),
	'popup.type.ctime' => $this->translate( 'admin', 'Created' ),
	'popup.type.mtime' => $this->translate( 'admin', 'Modified' ),
	'popup.type.editor' => $this->translate( 'admin', 'Editor' ),
];
...
<div class="list-view"
	data-domain="popup/type"
	data-filter="<?= $enc->attr( $this->session( 'aimeos/admin/jqadm/type/popup/filter', new \stdClass ) ) ?>"
	data-items="<?= $enc->attr( $this->get( 'items', map() )->call( 'toArray' )->all() ) ?>">

	<nav class="main-navbar">

		<span class="navbar-brand">
			<?= $enc->html( $this->translate( 'admin', 'Popup Types' ) ) ?>
	...
	<nav-search v-bind:show="search" v-on:close="search = false"
		...
		v-bind:filter="<?= $enc->attr( (object) $this->session( 'aimeos/admin/jqadm/type/popup/filter', new \stdClass ) ) ?>"
	...
	<?= $this->partial(
			...
			'page' => $this->session( 'aimeos/admin/jqadm/type/popup/page', [] )]
		);
	?>

	<form ref="form" class="list list-popup-type" method="POST"
				...
				<thead class="list-header">
					<tr>
						...
						<?= $this->partial(
								...
								['fields' => $fields, 'params' => $params, 'data' => $columnList, 'sort' => $this->session( 'aimeos/admin/jqadm/type/popup/sort' )]
							);
						?>
				...
				<tbody>
					...
					<?= $this->partial(
						$this->config( 'admin/jqadm/partial/listsearch', 'common/partials/listsearch-standard' ), [
							'fields' => array_merge( $fields, ['select'] ), 'filter' => $this->session( 'aimeos/admin/jqadm/type/popup/filter', [] ),
							'data' => [
								'popup.type.id' => ['op' => '=='],
								'popup.type.domain' => ['op' => '==', 'type' => 'select', 'val' => [
								...
								'popup.type.status' => ['op' => '==', 'type' => 'select', 'val' => [
								...
								'popup.type.code' => [],
								'popup.type.label' => [],
								'popup.type.position' => ['op' => '>=', 'type' => 'number'],
								'popup.type.ctime' => ['op' => '-', 'type' => 'datetime-local'],
								'popup.type.mtime' => ['op' => '-', 'type' => 'datetime-local'],
								'popup.type.editor' => [],
							]
						] );
					?>
