...
<div id="property" class="item-property tab-pane fade" role="tabpanel" aria-labelledby="property">
	...
	<div class="box vue" data-data="<?= $enc->attr( $this->get( 'propertyData', [] ) ) ?>">
		...
		<property-table
			v-bind:domain="'popup'" v-bind:siteid="`<?= $enc->js( $this->site()->siteid() ) ?>`" v-bind:tabindex="`<?= $enc->js( $this->get( 'tabindex' ) ) ?>`"
			v-bind:types="<?= $enc->attr( $this->get( 'propertyTypes', map() )->col( 'popup.property.type.label', 'popup.property.type.code' )->toArray() ) ?>"
			...
			v-bind:i18n="{
				...
				help: `<?= $enc->js( $this->translate( 'admin', 'Popup properties that are not shared with other popups' ) ) ?>`,
