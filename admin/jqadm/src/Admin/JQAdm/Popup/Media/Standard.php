...
namespace Aimeos\Admin\JQAdm\Popup\Media;

sprintf( 'media' ); // for translation


/**
 * Default implementation of popup media JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/** admin/jqadm/popup/media/name
	 * Name of the media subpart used by the JQAdm popup implementation
	 *
	 * Use "Myname" if your class is named "\Aimeos\Admin\Jqadm\Popup\Media\Myname".
	...
	/**
	 * Adds the required data used in the popup template
	 *
	...
	public function addData( \Aimeos\MW\View\Iface $view ) : \Aimeos\MW\View\Iface
	{
		...
		$search->setConditions( $search->compare( '==', 'media.type.domain', 'popup' ) );
		$search->setSortations( [$search->sort( '+', 'media.type.position' )] );

		$listSearch = $listTypeManager->filter( true )->slice( 0, 10000 );
		$listSearch->setConditions( $listSearch->compare( '==', 'popup.lists.type.domain', 'media' ) );
	...
	{
		/** admin/jqadm/popup/media/decorators/excludes
		 ...
		 *
		 *  admin/jqadm/popup/media/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see admin/jqadm/popup/media/decorators/global
		 * @see admin/jqadm/popup/media/decorators/local
		 */

		/** admin/jqadm/popup/media/decorators/global
		 ...
		 *
		 *  admin/jqadm/popup/media/decorators/global = array( 'decorator1' )
		 *
		 ...
		 * @category Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/popup/media/decorators/excludes
		 * @see admin/jqadm/popup/media/decorators/local
		 */

		/** admin/jqadm/popup/media/decorators/local
		 ...
		 *
		 *  admin/jqadm/popup/media/decorators/local = array( 'decorator2' )
		 *
		 ...
		 * @see admin/jqadm/popup/media/decorators/global
		 */
		return $this->createSubClient( 'popup/media/' . $type, $name );
	}
	 ...
	 *
	 * @param \Aimeos\MShop\Popup\Item\Iface $item Popup item including media reference
	 * @param array $listItems Media list items to be removed
	 * @return \Aimeos\MShop\Popup\Item\Iface Modified popup item
	 */
	protected function deleteMediaItems( \Aimeos\MShop\Popup\Item\Iface $item, array $listItems ) : \Aimeos\MShop\Popup\Item\Iface
	{
		$context = $this->getContext();
		$cntl = \Aimeos\Controller\Common\Media\Factory::create( $context );
		$manager = \Aimeos\MShop::create( $context, 'popup' );
		$search = $manager->filter();

		foreach( $listItems as $listItem )
		{
			$func = $search->make( 'popup:has', ['media', $listItem->getType(), $listItem->getRefId()] );
	...
	{
		/** admin/jqadm/popup/media/subparts
		 * List of JQAdm sub-clients rendered within the popup media section
		 *
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/popup/media/subparts', [] );
	}
	...
	protected function fromArray( \Aimeos\MShop\Popup\Item\Iface $item, array $data ) : \Aimeos\MShop\Popup\Item\Iface
	{
		...
		foreach( $data as $idx => $entry )
		{
			$listType = $entry['popup.lists.type'] ?? 'default';
			...
			$refItem->fromArray( $entry, true )->setDomain( 'popup' );
	...
	{
		/** admin/jqadm/popup/media/template-item
		 * Relative path to the HTML body template of the media subpart for popups.
		 *
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/popup/media/template-item';
		$default = 'popup/item-media-standard';
