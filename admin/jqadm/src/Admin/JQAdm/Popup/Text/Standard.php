...
namespace Aimeos\Admin\JQAdm\Popup\Text;

sprintf( 'text' ); // for translation


/**
 * Default implementation of popup text JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/** admin/jqadm/popup/text/name
	 * Name of the text subpart used by the JQAdm popup implementation
	 *
	 * Use "Myname" if your class is named "\Aimeos\Admin\Jqadm\Popup\Text\Myname".
	...
	{
		/** admin/jqadm/popup/text/decorators/excludes
		 ...
		 *
		 *  admin/jqadm/popup/text/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see admin/jqadm/popup/text/decorators/global
		 * @see admin/jqadm/popup/text/decorators/local
		 */

		/** admin/jqadm/popup/text/decorators/global
		 ...
		 *
		 *  admin/jqadm/popup/text/decorators/global = array( 'decorator1' )
		 *
		 ...
		 * @category Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/popup/text/decorators/excludes
		 * @see admin/jqadm/popup/text/decorators/local
		 */

		/** admin/jqadm/popup/text/decorators/local
		 ...
		 *
		 *  admin/jqadm/popup/text/decorators/local = array( 'decorator2' )
		 *
		 ...
		 * @see admin/jqadm/popup/text/decorators/global
		 */
		return $this->createSubClient( 'popup/text/' . $type, $name );
	}
	...
	{
		/** admin/jqadm/popup/text/subparts
		 * List of JQAdm sub-clients rendered within the popup text section
		 *
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/popup/text/subparts', [] );
	}
	...
	public function addData( \Aimeos\MW\View\Iface $view ) : \Aimeos\MW\View\Iface
	{
		...
		$listSearch->setConditions( $listSearch->compare( '==', 'popup.lists.type.domain', 'text' ) );
	...
	protected function fromArray( \Aimeos\MShop\Popup\Item\Iface $item, array $data ) : \Aimeos\MShop\Popup\Item\Iface
	{
		...
		foreach( $data as $idx => $entry )
		{
			...
			if( ( $listItem = $item->getListItem( 'text', $entry['popup.lists.type'], $entry['text.id'], false ) ) === null ) {
	...
	{
		/** admin/jqadm/popup/text/template-item
		 * Relative path to the HTML body template of the text subpart for popups.
		 *
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/popup/text/template-item';
		$default = 'popup/item-text-standard';
